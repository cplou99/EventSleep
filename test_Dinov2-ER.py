...
def extract_events_data(event_frame_folder, subjects, configs, batch_size, crop_bed, data_augmentation,
                        step_pixels, shuffle, device, chunk_len_ms, k, max_time, toy_data, reconstruction, full_sequence):
    X, y = None, None
    if full_sequence:
        config = configs[0]
        subject = subjects[0]
    ...
    for dir in dirs:
        ...
        if reconstruction:
            recons_dir = dir.replace('EventFrames', 'EventCameraReconstruction').split('.')[0]
            if not os.path.exists(recons_dir):
                continue
            imgs_paths = sorted(os.listdir(recons_dir))
            if len(imgs_paths) == 0:
                continue
            imgs = []
            for img_path in imgs_paths:
                rec = cv2.imread(f'{recons_dir}/{img_path}')
                rec = rec[:, :, 0] / 255
                rec = rec[np.newaxis, :, :, np.newaxis, np.newaxis]
                imgs.append(rec)
            imgs = np.concatenate(imgs, axis=0)
            X_dir = np.concatenate((X_dir, imgs), axis=-1)
...
@ex.capture
def extract_dino_features(dino_extractor, inputs):
    transform = torchvision.transforms.Compose([torchvision.transforms.Resize((252, 182)),
                                                torchvision.transforms.Normalize(mean=0.5, std=0.2)])
    img_tensor = transform(inputs)
    features = dino_extractor.forward_features(img_tensor.cuda())['x_norm_clstoken'].detach()
    return features


@ex.capture
def inference_model(model, dino_extractor, test_loader):
    y_pred, y_gt, outputs = [], [], []
    t0 = time.time()
    for inputs, targets in test_loader:
        features = extract_dino_features(dino_extractor, inputs)
        outputs = model(features)
        y_pred.append(outputs.argmax(-1).detach().cpu().numpy().tolist())
        y_gt.append(targets.detach().cpu().numpy().tolist())
        outputs.append(outputs.cpu().detach())
...
@ex.capture
def fit_laplace_classifier(model, dino_extractor, train_loader, batch_size, device):
    y_train, feats_train = [], []
    for inputs, targets in train_loader:
        outs = extract_dino_features(dino_extractor, inputs.to('cuda:0'))
    ...
    t0 = time.time()
    lap_classifier = Laplace(model, 'classification', subset_of_weights='all', hessian_structure='full')
...
@ex.capture
def inference_laplace_model(dino_extractor, lap_classifier, test_loader, device):
    y_pred, y_gt, probs = [], [], []
    t0 = time.time()
    for inputs, targets in test_loader:
        feats = extract_dino_features(dino_extractor, inputs.to('cuda:0'))
...
def test_model(folder_path, model, train_loader, checkpoint_paths, subjects_test, configs_test, batch_size, laplace,
    ...
    dino_extractor = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14').cuda()
    ...
    for checkpoint_path in checkpoint_paths:
        ...
        # Perform inference
        l_gt, l_pred, probs = inference_model(model, dino_extractor, test_loader)
        all_ens_probs.append(probs)

        if laplace:
            lap_classifier = fit_laplace_classifier(model, dino_extractor, train_loader)
            lap_gt, lap_pred, lap_probs = inference_laplace_model(dino_extractor, lap_classifier, test_loader)
