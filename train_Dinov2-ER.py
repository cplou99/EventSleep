...
def create_folder(toy_data):
    ...
    folder_path = os.path.join(f'{Path(os.getcwd())}/Models/Dinov2-ER/{folder_name_configs}/{file_name}')
...
def extract_events_data(event_frame_folder, subjects, configs, batch_size, crop_bed, data_augmentation,
                        step_pixels, shuffle, device, chunk_len_ms, k, max_time, toy_data, reconstruction, dirs=None):
...
@ex.capture
def load_model(in_channels, out_channels, lr, device, weight_decay):
    model = MyFCN(in_channels, out_channels)
...
def train_model(model, optimizer, metric, subjects_train, subjects_val, n_epochs, save_checkpoints, batch_size, lr,
    ...
    for n_ep in range(n_epochs):
        ...
        for in_val, t_val in val_loader:
            feat_val = extract_dino_features(dino_extractor, in_val)
            out_val = model(feat_val)
